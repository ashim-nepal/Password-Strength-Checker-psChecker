# Password-Strength-Checker-psChecker
# 

The Password Strength Checker allows users to:

- Check the strength of **multiple passwords in one run**
- Evaluate passwords based on: **Length, Uppercase characters, Lowercase characters, Numbers, Special characters**
- Receive **clear enhancement feedback**
- Get informative **responses**
- Apply a custom **owner override rule** for creatoor "Ashim Nepal" name-based passwords


## Acknowledgements

- I would also like to thank all the awesome coders and experts who have contributed different things about Python in internet from where I took guidance.
- I express my deepest appreciation and thank you to all those who helped me either directly or indirectly in this project.  


## Author

- [@ashim-nepal](https://www.github.com/ashim-nepal)

## FAQ

#### What is this project about?

This project focuses on password hygiene awareness, not hacking, and is designed for beginners who are into networking and security.


#### What programming language is used for Algorithm?

The specific language used for this project is Python and its regex library.

## Feedback

If you have any feedback, please reach out to me at nepalashim90@gmail.com


## 🚀 About Me
# Hi, I'm Ashim! 👋
I'm a Student, Learner, Programmer who is learning and working in Python and cyber security as my major!



## 🔗 Links
[![portfolio](https://img.shields.io/badge/my_portfolio-000?style=for-the-badge&logo=ko-fi&logoColor=white)](https://ashimnepal.com.np/)
[![linkedin](https://img.shields.io/badge/linkedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ashim-nepal)
[![twitter](https://img.shields.io/badge/twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/asnp_ash)

![Logo](https://github.com/ashim-nepal/images/blob/main/logoNewNobg.png?raw=true)
Ashim Nepal [AI]

## License

[MIT](https://choosealicense.com/licenses/mit/)

## Support

For support, email at nepalashim90@gmail.com


## Application Demo

I. Preview of project
![app-1](https://github.com/ashim-nepal/Password-Strength-Checker-psChecker/blob/main/outputImages/image1.png)

![app-2](https://github.com/ashim-nepal/Password-Strength-Checker-psChecker/blob/main/outputImages/image2.png)


Thats the psChecker, your feedback on this simple project would higly be appreciated!! Thank You for being all the way down here!


-The End-




